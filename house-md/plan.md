...
## Next step
Build **S01E03 — "Occam's Razor"** end-to-end, copying the locked template. Same per-episode workflow: confirm the HF file name first (likely `house_dataset_occams_razor.jsonl`; if it 404s, list the dataset tree at the HF API) → reconstruct the transcript in `~/dev/scratch` (never the repo) → distill → write `/house-md/s01e03.html` → run the fact-check workflow → add its card to `/house-md/index.html` → commit only the episode files. One episode per session, no hurry.

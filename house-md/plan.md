...
## Next step
Build **S01E05 — "Damned If You Do"** end-to-end, copying the locked template. Same per-episode workflow: confirm the HF file name first (likely `house_dataset_damned_if_you_do.jsonl`; if it 404s, list the dataset tree at the HF API — and watch for filename quirks like the space in `house_dataset occams_razor.jsonl`) → reconstruct the transcript in `~/dev/scratch` (never the repo; copy `parse.py` and change only the input filename) → distill, cross-checking the diagnosis **and** the finale against documented episode facts (the transcript can cut off before the reveal, as it did for S01E03 and S01E04) → write `/house-md/s01e05.html` → run the fact-check workflow → add its card to `/house-md/index.html` → commit only the episode files. One episode per session, no hurry.
